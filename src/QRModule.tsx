...
function QRImage({code,size=150}:{code:string;size?:number}){
  ...
  useEffect(()=>{let alive=true;QRCode.toDataURL(qrValue(code),{width:size,margin:2,errorCorrectionLevel:'M'}).then(v=>alive&&setSrc(v));return()=>{alive=false}},[code,size]);
...
export default function QRModule({products,onScan}:Props){
  ...
  return <section className="panel qr-module">
    ...
    {scanner&&<QRScanner onClose={()=>setScanner(false)} onScan={onScan}/>} 
  </section>
}
